...
import { Save, ArrowLeft, Loader2, TrendingUp } from 'lucide-react';
...
function CreateTrackerContent() {
  ...
  const searchParams = useSearchParams(); // Hook penyebab error
  ...
  const [formData, setFormData] = useState({
    ...
    'PROBLEM': 'Nihil', // Default Value sesuai request
    'TEAM': '',
    'STATUS': 'Done',   // Default Value sesuai request
    'BTS': '',
    'DEVICE': '',
    'ISP': '',
    'REASON': ''
  ...
  const handleSave = async (e: any) => {
    ...
    if (!formData['SUBJECT BERLANGGANAN']) {
      alert('Nama Subject / Pelanggan wajib diisi!');
      setSaving(false);
      return;
    }
    ...
    if (selectedTable === 'Berlangganan 2026') {
        const confirmNext = confirm('Tracker Disimpan! \nLanjut input Data Client Baru?');
        if (confirmNext) {
            const name = encodeURIComponent(formData['SUBJECT BERLANGGANAN']);
            router.push(`/clients/create?name=${name}`);
        } else {
            router.push('/tracker'); router.refresh();
        }
        ...
        await supabase.from('Data Client Corporate')
          .update({ 'STATUS': newStatus })
          .ilike('Nama Pelanggan', `%${targetName}%`);
          
        alert(`Sukses! Status client '${targetName}' diupdate jadi '${newStatus}'.`);
        router.push('/tracker'); router.refresh();
    } else {
