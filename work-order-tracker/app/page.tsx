...
import { 
  Users, Server, Activity, TrendingUp, 
  ArrowUpRight, Clock, ShieldCheck, 
  Sun, Moon, CalendarDays,
  Zap, Save, Edit3, PieChart, AlertTriangle 
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  async function fetchDashboardData() {
    ...
    try {
      ...
      // 2. VLAN Stats
      const vlanPromises = [
        supabase.from('Daftar Vlan 1-1000').select('NAME'),
        supabase.from('Daftar Vlan 1000+').select('NAME'),
        supabase.from('Daftar Vlan 2000+').select('NAME'),
        supabase.from('Daftar Vlan 3000+').select('NAME'),
      ];
      const vlanResults = await Promise.all(vlanPromises);
      ...
      const growthMonth = pasang?.filter(r => r.TANGGAL?.startsWith(curMonth)).length || 0;
      ...
      setChartSeries([{ name: 'Pasang Baru', data: sPasang }, { name: 'Putus', data: sPutus }]);
      ...
      setStats({
        ...
        growthMonth, logsToday: countToday || 0
      });
      setRecentLogs(logs || []);
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen font-sans">
      ...
      {/* --- ALERT BANNER (KOTAK MERAH) --- */}
      {/* Menggantikan Header Statis Lama */}
      <AlertBanner />
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="space-y-4">
              <div className="bg-amber-50 p-3 rounded-lg border border-amber-100">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs font-bold text-amber-700 uppercase">Shift Pagi (08.00 - 16.00)</p>
                </div>
                <div className="flex gap-2">
                  {morningSquad.map((name, i) => (
                    <span key={i} className="px-3 py-1 bg-white text-slate-700 text-sm font-bold rounded shadow-sm border border-amber-200 flex-1 text-center">{name}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-indigo-50 p-3 rounded-lg border border-indigo-100">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs font-bold text-indigo-700 uppercase">Shift Siang (14.00 - 22.00)</p>
                </div>
                <div className="flex gap-2">
                  {afternoonSquad.map((name, i) => (
                    <span key={i} className="px-3 py-1 bg-white text-slate-700 text-sm font-bold rounded shadow-sm border border-indigo-200 flex-1 text-center">{name}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col flex-1 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto max-h-[300px] divide-y divide-slate-50">
              {loading ? <p className="p-4 text-center text-xs text-slate-400">Loading...</p> : 
               recentLogs.map((log) => (
