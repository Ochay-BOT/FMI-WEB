    ...
    const rules = [
      {
        ...
        targetCol: 'SUBJECT BERLANGGANAN',
      ...
      },
      {
        ...
        targetCol: 'SUBJECT BERHENTI BERLANGGANAN',
        label: 'Berhenti',
        color: 'red'
      },
      {
        ...
        targetCol: 'SUBJECT BERHENTI SEMENTARA',
        label: 'Cuti',
        color: 'orange'
      },
      {
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-300">
          ...
          {/* UPDATE: Ukuran diubah jadi 'max-w-4xl' (Sesuai Kotak Merah) */}
          <div className="bg-slate-50 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[85vh] border border-slate-700">
            ...
            <div className="bg-slate-900 px-6 py-5 flex items-center justify-between shrink-0 border-b border-slate-700">
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 border border-slate-700"
                >
                ...
                <button 
                  ...
                  className="p-1.5 bg-slate-800 hover:bg-red-900/50 text-slate-400 hover:text-red-400 rounded-lg transition-all border border-slate-700"
                >
                  <X size={20} />
                </button>
              </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {missingItems.map((item, idx) => {
                    ...
                    return (
                        <div 
                            key={idx} 
                            className={`bg-white rounded-lg p-0 shadow-sm hover:shadow-md transition-all border-l-[4px] ${theme.border} border-t border-r border-b border-slate-200 flex flex-col items-stretch group overflow-hidden`}
                        >
                            <div className="p-4 flex flex-row items-start gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded border uppercase tracking-wider ${theme.badge}`}>
                                            {item.type}
                                        </span>
                                    </div>
                                    
                                    <h3 className="text-base font-bold text-slate-800 leading-snug mb-2 group-hover:text-blue-700 transition-colors">
                                      {item.subject}
                                    </h3>
                                    
                                    <div className="flex items-center gap-1.5 text-xs">
                                      <span className="text-slate-400 flex items-center gap-1">
                                        <Info size={12} /> Missing di:
                                      </span>
                                      <span className="font-mono font-bold text-slate-600 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200">
                                        {item.targetTable}
                                      </span>
                                    </div>
                                </div>

                                {/* TOMBOL ACTION - Dibuat disamping agar compact */}
                                <div className="shrink-0 self-center">
                                    <button 
                                        onClick={() => handleFixData(item.subject)}
                                        className={`px-4 py-2 rounded-md font-bold text-white text-xs shadow-sm flex items-center gap-1.5 transition-transform active:scale-95 ${theme.btn}`}
                                    >
            ...
            <div className="bg-white px-6 py-3 border-t border-slate-200 flex justify-between items-center text-xs text-slate-400">
              ...
              <button 
                onClick={() => setIsOpen(false)}
                className="px-4 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-lg transition-colors"
              >
                Tutup (Esc)
              </button>
            </div>
